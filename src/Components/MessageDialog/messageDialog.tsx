...
const YieldebaranMessage: React.FC<Props> = (props: Props) => {
  return props.isOpen ? (
    <Modal
      ...
      closeTimeoutMS={50}
    >
      <YieldebaranMessageTitle
        onRequestClose={props.onRequestClose}
        darkMode={props.className?.includes('mymodal-dark') ? true : false}
      >
        {props.contentLabel}
      </YieldebaranMessageTitle>
...
const YieldebaranMessageTitle: FCC<TitleProps> = ({ onRequestClose, darkMode, children }) => {
  return (
  ...
  return (
    <div
      className={`yieldebaran-message-item ${disabled ? 'yieldebaran-message-item-disabled' : ''} ${
        hover && !disabled ? 'yieldebaran-message-item-hover' : ''
      }`}
      onClick={onClick}
    >
      {children}
